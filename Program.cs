namespace DiakConsoleApp
{
    internal class Program
    {
        ...
        void Harmadik()
        {
            ...
            Console.WriteLine($"\tA legtobb oldalt olvaso diák: {diakok.OrderByDescending(diak => diak.OldalSzam).First().Nev}");
        }
        ...
        void Hatodik()
        {
            ...
            Console.WriteLine($"\tA legtobben ebben az evfolyamban jarnak: {diakok.GroupBy(diak => diak.Evfolyam).OrderByDescending(group => group.Count()).First().First().Evfolyam}");
        }
        ...
        void Nyolcadik()
        {
            ...
            diakok.Where(diakok => diakok.Evfolyam == 12).OrderBy(diakok => diakok.Nev).ToList().ForEach(diak => Console.WriteLine($"\t{diak.Nev}"));
        }
        ...
        void Tizedik()
        {
            ...
            Console.WriteLine($"\tA legtobbet olvaso osztaly: {diakok.GroupBy(diak => diak.Osztaly).OrderByDescending(group => group.Average(diak => diak.OldalSzam)).First().Key}");
        }
        ...
        void Tizenotodik()
        {
            File.WriteAllLines(@"../../../intervallumok.txt", diakok.GroupBy(diak => diak.OldalSzam / 100).OrderBy(group => group.Key).Select(group => $"{group.Key * 100 + 1}-{group.Key * 100 + 100} oldal: {group.Count()} diak"));
        }
